    ...
    <div class="admin-shell">
      <div class="hero-card">
        <div>
          ...
          <p class="hero-copy">
            Gestiona profesores, alumnos, bloqueos, grados y vínculos
            docente-alumno.
          </p>
        </div>
        <button
      ...
      <template v-else>
          ...
          <table class="data-table">
            ...
            <tbody>
              <tr v-for="teacher in filteredTeachers" :key="teacher.user.id">
                ...
                <td data-label="Email" class="cell-muted">
                  {{ teacher.user.email }}
                </td>
                <td data-label="Roles">
                ...
                <td data-label="Asignados" class="cell-center">
                  {{
                    teacherAssignments[practiqUserId(teacher.user)]?.length || 0
                  }}
                </td>
                <td data-label="Estado">
                  <span
                    class="teacher-state"
                    :class="
                      teacher.profile?.academic_status === 'blocked'
        ...
        <section v-if="pendingProfiles.length" class="panel-card">
          ...
          <table class="data-table">
            ...
            <tbody>
              <tr
                v-for="item in filteredPendingProfiles"
                :key="item.user.id"
                class="row--pending"
              >
            ...
            <tbody>
              <tr v-for="item in filteredStudents" :key="item.user.id">
                ...
                <td data-label="Docentes">
                  <div class="chip-row chip-row--inline">
                    <span
                      v-for="teacher in studentTeachers[
                        practiqUserId(item.user)
                      ]"
                      :key="teacher.id"
                      class="soft-chip soft-chip--teacher"
                      >{{ teacher.name }}</span
                    >
                    <span
                      v-if="!studentTeachers[practiqUserId(item.user)]?.length"
                      class="detail-empty"
                      >Sin docente</span
                    >
                  </div>
                ...
                <td data-label="Grados">
                  <div class="chip-row chip-row--inline">
                    <span
                      v-for="grade in userGrades[practiqUserId(item.user)]"
                      :key="grade.id"
                      class="soft-chip soft-chip--grade"
                      >{{ grade.name }}</span
                    >
                    <span
                      v-if="!userGrades[practiqUserId(item.user)]?.length"
                      class="detail-empty"
                      >Sin grado</span
                    >
                  </div>
                ...
                <td data-label="Estado">
                  <span
                    class="status-pill"
                    :class="
                      item.profile?.academic_status === 'blocked' ||
                      !item.user.is_active
                        ? 'status-pill--blocked'
                        : 'status-pill--ok'
                    "
          ...
          <div class="modal-card">
            ...
            <div class="modal-body">
              <div class="detail-card">
                ...
                <div class="chip-row">
                  <span
                    v-for="teacher in currentStudentTeachers"
                    :key="teacher.id"
                    class="soft-chip soft-chip--teacher"
                  >
                    {{ teacher.name }}
                    <button
                      ...
                      class="chip-action"
                      @click="
                        unassignTeacher(teacher.id, currentEditingStudentId)
                      "
                    >
                      ×
                    </button>
                  ...
                  <span
                    v-if="!currentStudentTeachers.length"
                    class="detail-empty"
                    >Sin docente asignado</span
                  >
                </div>
                <div class="action-row">
                  <select
                    v-model="teacherSelection[currentEditingStudentId]"
                    class="form-select"
                  >
                    ...
                    <option
                      ...
                      :key="teacher.user.id"
                      :value="practiqUserId(teacher.user)"
                    >
                  ...
                  <button
                    class="btn btn-secondary btn-sm"
                    type="button"
                    @click="assignTeacher(currentEditingStudentId)"
                  >
                    Asignar
                  </button>
                </div>
              ...
              <div class="detail-card">
                ...
                <div class="chip-row">
                  <span
                    v-for="grade in currentStudentGrades"
                    :key="grade.id"
                    class="soft-chip soft-chip--grade"
                  >
                    {{ grade.name }}
                    <button
                      type="button"
                      class="chip-action"
                      @click="removeGrade(grade.id, currentEditingStudentId)"
                    >
                      ×
                    </button>
                  </span>
                  <span v-if="!currentStudentGrades.length" class="detail-empty"
                    >Sin grado</span
                  >
                </div>
                <div class="action-row">
                  <select
                    v-model="gradeSelection[currentEditingStudentId]"
                    class="form-select"
                  >
                  ...
                  <button
                    class="btn btn-secondary btn-sm"
                    type="button"
                    @click="assignGrade(currentEditingStudentId)"
                  >
                    Vincular
                  </button>
                </div>
              ...
              <div class="action-row action-row--split">
                <button
                  class="btn"
                  :class="
                    editingStudent.profile?.academic_status === 'blocked' ||
                    !editingStudent.user.is_active
                      ? 'btn-secondary'
                      : 'btn-danger'
                  "
                  type="button"
