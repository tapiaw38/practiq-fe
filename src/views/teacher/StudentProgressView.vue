      ...
      <template v-if="loadingAll">
        ...
        <div class="tab-bar">
          <div
            v-for="i in 4"
            :key="i"
            class="tab-btn tab-btn--skeleton"
      ...
      <template v-else>
          ...
          <div v-else class="progress-grid">
            ...
            >
              ...
              <div class="progress-meta">
                <span
                  ><i class="pi pi-check"></i> {{ p.correct_attempts }}/{{
                    p.total_attempts
                  }}
                  correctas</span
                >
                <span
                  ><i class="pi pi-trophy"></i> Nivel
                  {{ p.current_level }}</span
                >
                <span v-if="p.last_practiced_at"
                  ><i class="pi pi-calendar"></i>
                  {{ formatDate(p.last_practiced_at) }}</span
                >
              </div>
            </div>
          </div>
        ...
        <div v-if="activeTab === 'courses'" class="tab-content">
          ...
          <div v-else class="course-progress-list">
            <div v-for="c in courses" :key="c.id" class="course-progress-item">
              ...
              <div
                v-if="(courseProgressMap[c.id] || []).length"
                class="course-topic-list"
              >
                <div
                ...
                >
                  ...
                  <span
                    class="course-topic-score"
                    :class="masteryClass(p.mastery_score)"
                  >
                    {{ p.mastery_score.toFixed(0) }}%
                  </span>
                  <span class="course-topic-level"
                    >Nivel {{ p.current_level }}</span
                  >
                </div>
              </div>
              <div v-else class="course-topic-empty">
                Sin actividad en este curso.
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-if="activeTab === 'sheets'" class="tab-content">
          <div class="tab-section-head">
            ...
            <span class="tab-section-chip"
              >{{ filteredSheets.length }} hojas</span
            >
          </div>
          <!-- Course selector -->
            ...
            >
              <div class="sheet-card__left">
                ...
                <div class="sheet-meta">
                  Nivel {{ sheet.level }} &bull;
                  {{ sheet.exercises.length }} ejercicios &bull; Estilo:
                  {{ sheet.test_style === "canvas" ? "Canvas" : "Teclado" }}
                </div>
              </div>
              <button
              ...
              >
                <span
                  v-if="loadingAttempts && selectedSheet?.id === sheet.id"
                  class="spinner spinner-sm"
                ></span>
                <template v-else
                  ><i class="pi pi-eye"></i> Ver intentos</template
                >
              </button>
            </div>
          </div>
          ...
          <Transition name="slide-up">
                    ...
                    >
                      ...
                      <td class="answer-cell" data-label="Respuesta">
                        <template v-if="getAttemptImageSrc(a)">
                          <button
                            class="btn btn-secondary btn-sm"
                            @click="openAttemptImage(getAttemptImageSrc(a)!)"
                          >
            ...
            <div
              v-else-if="
                selectedSheet && !loadingAttempts && attempts.length === 0
              "
              class="attempts-panel"
            >
              <div class="attempts-panel__head">
        ...
        <div v-if="activeTab === 'notebooks'" class="tab-content">
          <div class="tab-section-head">
            ...
            <span class="tab-section-chip"
              >{{ filteredNotebooks.length }} cuadernos</span
            >
          </div>
          <div class="filter-row">
            <label class="filter-label">Filtrar por curso</label>
            <select
              v-model="selectedCourseIdNB"
              class="form-select"
              @change="onCourseChangeNB"
            >
              <option value="">Todos los cursos</option>
              <option v-for="c in courses" :key="c.id" :value="c.id">
                {{ c.title }}
            ...
            >
              ...
              <div class="notebook-card__body">
                ...
                <div class="notebook-meta">
                  Nivel {{ nb.level }} &bull;
                  {{ nb.pages?.length ?? 0 }} páginas
                </div>
                <div class="notebook-desc" v-if="nb.description">
                  {{ nb.description }}
                </div>
              </div>
            ...
            <div v-if="selectedNotebook" class="notebook-viewer">
              ...
              <div v-else>
                <div class="nb-pages-summary">
                  <span
                    >{{ submittedPages }} /
                    {{ selectedNotebook.pages?.length ?? 0 }} páginas
                    completadas</span
                  >
                </div>
                <div class="nb-pages-grid">
                  ...
                  >
                    <div class="nb-page-header">
                      <span class="nb-page-num"
                        >Pág. {{ page.page_number }}</span
                      >
                      <span class="nb-page-title">{{
                        page.title || "Sin título"
                      }}</span>
                      <span
                        class="nb-page-status"
                        :class="
                          page.submission ? 'status--done' : 'status--pending'
                        "
                      >
                    ...
                    <div class="nb-page-content">
                      ...
                      <div class="nb-content-block">
                        ...
                        >
                          <img
                            :src="page.content_data"
                            class="nb-canvas-img"
                            alt="Contenido"
                          />
                        </template>
                        <template v-else>
                          <p class="nb-text-content">
                            {{ page.content_data || "—" }}
                          </p>
                        </template>
                      ...
                      <div class="nb-content-block" v-if="page.submission">
                        <div class="nb-content-label nb-content-label--student">
                          Respuesta del alumno
                        </div>
                        <div
                          v-if="page.submission.ai_is_correct !== undefined"
                          class="nb-ai-row"
                        >
                          <span
                            class="nb-ai-badge"
                          ...
                          <span
                            v-if="page.submission.ai_reviewed_at"
                            class="nb-ai-date"
                            >{{
                              formatDate(page.submission.ai_reviewed_at)
                            }}</span
                          >
                        </div>
                        <template v-if="page.submission.canvas_data">
                          <img
                            :src="page.submission.canvas_data"
                            class="nb-canvas-img"
                            alt="Respuesta del alumno"
                          />
                        </template>
                        <template v-else-if="page.submission.answer_text">
                          <p class="nb-text-content">
                            {{ page.submission.answer_text }}
                          </p>
                        </template>
                        <p
                          v-else
                          class="nb-text-content nb-text-content--empty"
                        >
                          Sin respuesta guardada
                        </p>
                        <p
                          v-if="formatAIFeedback(page.submission.ai_feedback)"
                          class="nb-ai-feedback"
                        >
                          {{ formatAIFeedback(page.submission.ai_feedback) }}
                        </p>
                      </div>
                      <div
                        class="nb-content-block nb-content-block--empty"
                        v-else
                      >
                        <div class="nb-content-label">Respuesta del alumno</div>
                        <p class="nb-text-content nb-text-content--empty">
                          El alumno no ha completado esta página.
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          <div class="pdf-modal-card">
            ...
            <div class="pdf-modal-body">
              <div class="form-group">
                ...
                <p class="form-hint">
                  Dejar vacio para incluir todo el historial
                </p>
              </div>
              <div class="form-group">
            ...
            <div class="pdf-modal-footer">
              <button class="btn btn-secondary" @click="showPdfModal = false">
                Cancelar
              </button>
              <button
              ...
              >
                ...
                ></span>
                <template v-else
                  ><i class="pi pi-download"></i> Descargar</template
                >
              </button>
            </div>
          </div>
...
<style scoped>
  ...
  .mastery--high .course-topic-bar-fill {
    background: var(--color-success);
  }
  .course-topic-bar-fill.mastery--mid,
  .mastery--mid .course-topic-bar-fill {
    background: var(--color-warning);
  }
  .course-topic-bar-fill.mastery--low,
  .mastery--low .course-topic-bar-fill {
  ...
  .mastery--high .course-topic-score {
    color: var(--color-success-dark);
  }
  .mastery--mid .course-topic-score {
    color: var(--color-warning-dark);
  }
  .mastery--low .course-topic-score {
  ...
  .mastery--high .mastery-badge,
  .mastery-badge.mastery--high {
    background: var(--color-success-bg);
    color: var(--color-success-dark);
  }
  .mastery--mid .mastery-badge,
  .mastery-badge.mastery--mid {
    background: var(--color-warning-bg);
    color: var(--color-warning-dark);
  }
  .mastery--low .mastery-badge,
  ...
  .mastery--high .progress-bar-fill {
    background: var(--color-success);
  }
  .progress-bar-fill.mastery--mid,
  .mastery--mid .progress-bar-fill {
    background: var(--color-warning);
  }
  .progress-bar-fill.mastery--low,
  .mastery--low .progress-bar-fill {
    background: var(--color-error);
  }
  ...
  .sheet-type-badge.level_test {
    background: var(--fill-primary-soft);
    color: var(--practiq-violet);
  }
  .sheet-type-badge.practice {
    background: var(--color-info-bg);
    color: var(--color-info-dark);
  }
  ...
  .nb-page-card--empty {
    border-color: rgba(var(--surface-border-rgb), 0.2);
  }
  ...
  .nb-content-block:last-child {
    border-right: none;
  }
  .nb-content-block--empty {
    background: var(--surface-subtle);
  }
  ...
  @media (max-width: 1024px) {
    ...
    .course-topic-row {
      grid-template-columns: 140px 1fr 52px 60px;
    }
    .progress-grid {
