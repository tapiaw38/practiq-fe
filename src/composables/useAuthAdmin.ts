...
export const useAuthAdmin = () => {
  ...
  const loadUsers = async (params?: { limit?: number; offset?: number; role?: string }) => {
    try {
  ...
  const updateUser = async (
    id: string,
    params: Partial<Pick<AuthApiUser, 'first_name' | 'last_name' | 'email' | 'is_active' | 'verified_email'>>
  ) => {
    try {
