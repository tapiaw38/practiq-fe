...
export const useStrategy = () => {
  ...
  const { strategies, currentStrategy, courseAssignments, loading } =
    storeToRefs(store);
  ...
  const loadStrategies = async () => {
    try {
      ...
      toast.add({
        severity: "error",
        summary: "Error",
        detail: "No se pudieron cargar las estrategias",
        life: 3000,
      });
      throw error;
    }
  ...
  const createStrategy = async (params: CreateStrategyParams) => {
    try {
      ...
      toast.add({
        severity: "error",
        summary: "Error",
        detail: "No se pudo crear la estrategia",
        life: 3000,
      });
      throw error;
    }
  ...
  const updateStrategy = async (id: string, params: UpdateStrategyParams) => {
    try {
      ...
      toast.add({
        severity: "error",
        summary: "Error",
        detail: "No se pudo actualizar la estrategia",
        life: 3000,
      });
      throw error;
    }
  ...
  const deleteStrategy = async (id: string) => {
    try {
      ...
      toast.add({
        severity: "error",
        summary: "Error",
        detail: "No se pudo eliminar la estrategia",
        life: 3000,
      });
      throw error;
    }
  ...
  const assignStrategyToCourse = async (
    courseId: string,
    strategyId: string,
  ) => {
    try {
      ...
      toast.add({
        severity: "success",
        summary: "Éxito",
        detail: "Estrategia asignada correctamente",
        life: 3000,
      });
      return assignment;
    } catch (error) {
      toast.add({
        severity: "error",
        summary: "Error",
        detail: "No se pudo asignar la estrategia",
        life: 3000,
      });
      throw error;
    }
