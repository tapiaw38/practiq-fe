...
export const useProgress = () => {
  ...
  const { myProgress, courseProgress, studentProgress, studentAttempts, loading } = storeToRefs(store)
  ...
  const loadStudentCourseProgress = async (studentId: string, courseId: string) => {
    try {
  ...
  const loadStudentCourseProgressNew = async (studentId: string, courseId: string) => {
    try {
  ...
  const downloadReport = async (studentId: string, options?: { from?: string; to?: string; courseId?: string }) => {
    try {
