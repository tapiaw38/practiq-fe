...
export const useAssignment = () => {
  ...
  const { teacherStudents, studentTeachers, myStudents, loading } =
    storeToRefs(store);
  ...
  const assignTeacher = async (teacherId: string, studentId: string) => {
    try {
      ...
      toast.add({
        severity: "error",
        summary: "Error",
        detail: "No se pudo asignar el profesor",
        life: 3000,
      });
      throw error;
    }
  ...
  const unassignTeacher = async (teacherId: string, studentId: string) => {
    try {
      ...
      toast.add({
        severity: "error",
        summary: "Error",
        detail: "No se pudo desvincular el profesor",
        life: 3000,
      });
      throw error;
    }
  ...
  const loadMyStudents = async () => {
    try {
      ...
      toast.add({
        severity: "error",
        summary: "Error",
        detail: "No se pudieron cargar los alumnos asignados",
        life: 3000,
      });
      throw error;
    }
