...
export const useProfile = () => {
  ...
  const syncProfile = async (params: SyncProfileParams) => {
    try {
      ...
      toast.add({ severity: 'error', summary: 'Error', detail: 'No se pudo sincronizar el perfil', life: 3000 })
      throw error
    }
  }
  ...
  const updateAssistantConfigById = async (id: string, params: AssistantConfigParams) => {
    try {
  ...
  const updateAcademicStatusById = async (id: string, params: AcademicStatusParams) => {
    try {
      return await store.updateAcademicStatusById(id, params)
    } catch (error) {
      toast.add({ severity: 'error', summary: 'Error', detail: 'No se pudo cambiar el estado académico', life: 3000 })
      throw error
    }
  }
